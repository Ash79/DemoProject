  ...
  // we want to test the function that is calling the external service.
    ...
    describe('and there is an record in the data with that ID', () => {
      before(() => provider.addInteraction({
        state: 'Has a record with ID '+expectedRecords[0]['userid'],
        uponReceiving: 'a request for a record with ID '+expectedRecords[0]['userid'],
        withRequest: {
          method: 'GET',
          path: BASE_PATH+'/'+expectedRecords[0]['userid']
        },
        willRespondWith: {
          status: 200,
          headers: {
      ...
      it('returns the record', done => {
        client.getuserById(expectedRecords[0]['userid'],(error,response)=>{
          var parsedData = JSON.parse(response.body);
          expect(parsedData).to.have.deep.property('userid', expectedRecords[0]['userid']);
    ...
    describe('and there no records in the database', () => {
      ...
      it('returns a 404', done => {
        client.getuserById('NonExisting',(error,response)=>{
      ...
      describe('and there is a record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['userid'],
          uponReceiving: 'a request to update a record with ID '+expectedRecords[0]['userid'],
          withRequest: {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
        ...
        it('returns the updated record', done => {
          client.putuser(expectedRecords[0]['userid'], expectedRecords[0], (error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('userid', expectedRecords[0]['userid']);
            done();
          })
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.putuser('NonExisting', expectedRecords[0], (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
      ...
      describe('and the record abides by the required schema', () => {
        ...
        it('returns the newly added record', done => {
          client.postuser(expectedRecords[0],(error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('userid', expectedRecords[0]['userid']);
            done();
          });
      ...
      describe('and the record is invalid with respect to the schema', () => {
        ...
        it('returns 400 error', done => {
          client.postuser({ 'dummyKey': 'dummyValue' },(error,response)=>{
      ...
      delete semiRecord['userid'];
      describe('and there is an record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['userid'],
          uponReceiving: 'a request to semi-update a record with ID '+expectedRecords[0]['userid'],
          withRequest: {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json'
        ...
        it('returns the updated record', done => {
          client.patchuser(expectedRecords[0]['userid'], semiRecord, (error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('userid', expectedRecords[0]['userid']);
            done();
          })
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.patchuser('NonExisting', semiRecord, (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
      ...
      describe('and there is a record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['userid'],
          uponReceiving: 'a request to delete a record with ID '+expectedRecords[0]['userid'],
          withRequest: {
            method: 'DELETE',
            path: BASE_PATH+'/'+expectedRecords[0]['userid']
          },
        ...
        it('returns 204', done => {
          client.deleteuser(expectedRecords[0]['userid'], (error,response)=>{
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.deleteuser('NonExisting', (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
